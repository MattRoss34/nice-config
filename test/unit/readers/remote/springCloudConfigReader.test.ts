...
import { springCloudConfigReader } from '../../../../src/readers/remote';
...
describe('SpringCloudConfigServiceImpl', function() {
	...
	describe('#getConfigFromServer()', function() {
		...
		beforeEach(async function() {
			gatewayStub = sandbox.stub(SpringCloudConfigGateway.prototype, 'getConfigFromServer');
			bootstrapConfig = {
				spring: { cloud: { config: {
					enabled: true,
		...
		it('should skip cloud config when not enabled', async function() {
			...
			const getConfigFromServer: Promise<ConfigObject> = springCloudConfigReader(bootstrapConfig);
